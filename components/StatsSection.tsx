      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        >
          <h2 className="text-4xl md:text-6xl font-display italic uppercase text-white mb-4">
            {content.homepage.statsTitle.split(' ').slice(0, -1).join(' ')} <span className="text-primary-yellow">{content.homepage.statsTitle.split(' ').slice(-1)}</span>
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12">
          {content.homepage.stats.map((stat: Stat, index: number) => (
            <motion.div
              ...
              className="text-center p-8 rounded-card bg-white/5 border border-white/10 hover:border-primary-yellow/50 transition-all duration-300 group"
            >
              <motion.div
                whileHover={{ scale: 1.05 }}
                className="text-5xl md:text-6xl lg:text-7xl font-display italic uppercase text-primary-yellow mb-4 font-bold"
              >
